...
class PracticeController extends Controller
{
    ...
    */
    ...
    {
        #dump(Book::orderByAsc('title')->get());
        $r = Book::where('title', 'LIKE', '%Harry%')->get();
        dump($r);
    }
    ...
    /**
    * https://github.com/susanBuck/dwa15-fall2017/issues/232
    */
    public function practice32()
    {
        # Instantiate our custom validation rule to test
        $validImage = new ValidImage();

        # The paths we'll test
        # The boolean value for each path indicates whether the validator should pass/fail
        $tests = [
            'http://foobooks.dwa15.me/images/laravel-foobooks-logo@2x.png' => true, # Valid image
            'http://foobooks.dwa15.me/images/laravel-foobooks-logo@2x.text' => false, # Invalid image
            'http://foobooks.dwa15.me/images/foobar.png' => false, # Invalid image
            'foobar.png' => false,
        ];

        # Run each test
        foreach ($tests as $path => $expected) {
            # Invoke the validator for this path
            $results = $validImage->passes('image', $path);

            # Compare the results
            if ($results == $expected) {
                $isOrIsNot = ($expected) ? ' is' : ' is NOT';
                dump('✓ Test passed; detected that '.$path.$isOrIsNot.' a valid image URL.');
            }
        }
    }
    ...
    /**
    * Example for Chen
    * How to sort results based on pivot relationship
    */
    public function practice31()
    {
        $books = Book::with('author')->get()->sortBy('author.last_name');

        # Debugging output to check results
        foreach ($books as $book) {
            dump($book->title.' by '.$book->author->last_name.', '.$book->author->first_name);
        }
    }


    /**
    * https://github.com/susanBuck/dwa15-fall2017/issues/208
    */
    public function practice30b(Request $request)
    {
        # Instantiate the validator
        $v = Validator::make($request->all(), []);

        # If the author_id is not null, then apply `numeric` validation rule
        # https://laravel.com/docs/5.5/validation#conditionally-adding-rules "Complex Conditional Validation"
        $v->sometimes('author_id', 'numeric', function ($input) {
            return !is_null($input->author_id);
        });

        $v->validate();

        echo "If you see this, validation passed. <a href='".url()->previous()."'>Back</a>";
    }

    public function practice30()
    {
        return view('practice.30');
    }



    /**
    * https://github.com/susanBuck/dwa15-fall2017/issues/202
    */
    public function practice29()
    {
        # Approach A. (2 queries)
        # Get all the books and their tags, then filter out books with the tag classic
        $allBooks = Book::with('tags')->get();

        $books = $allBooks->reject(function ($book, $key) {
            # Returns True if any of the tags are classic
            # When True is returned, the book is "rejected", i.e. filtered out
            return $book->tags->contains('name', 'classic');
        });

        dump($books->toArray());

        # Approach B. (2 queries total)
        # Get the ids of all the books we want to exclude (books with the tag `classic`)
        $idsOfBooksToExclude = Book::whereHas('tags', function ($query) {
            $query->where('name', 'like', 'classic');
        })->pluck('id')->toArray();

        # Now that we know this info, get all the books *except* the ones we found in the previous step
        $books = Book::whereNotIn('id', $idsOfBooksToExclude)->get();
    ...
    /**
    * https://github.com/susanBuck/dwa15-fall2017/issues/200
    */
    public function practice28()
    {
        # Mock data representing the kind of data you'd get from the request
        $newBookData = collect([
            "title" => "The Great Gatsby!", # <-- Note the difference
            "published" => 1925,
        ...
        # Get a book to compare the above data with
        $book = Book::find(1);

        # Look for differences
        $differences = $newBookData->diffAssoc($book);

        if (count($differences) == 0) {
            dump('No differences');
        } else {
            # Output the differences
            dump($differences->all());
        }
    }


    /**
    * https://github.com/susanBuck/dwa15-fall2017/issues/191
    */
    public function practice27()
    {
        $book = Book::find(1);
        echo gettype($book); # A Collection is an object
        echo $book->title; # Given that, a Collection can obviously be treated as an object
        echo $book; # A Collection can also be treated as a string
        echo $book['title']; # Or an array

        # But can we treat/use a Collection as a Boolean value? Yup:
        echo $book == true; # 1
        echo $book == false; # 0
    }
